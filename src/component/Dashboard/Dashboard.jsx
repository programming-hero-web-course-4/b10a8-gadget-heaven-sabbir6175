...
import { getStoredAddToCart, getStoredWishList} from '../Add To LocalStorage/AddtoStorage';
...
const Dashboard = () => {
    ...
    // delete cart items dashboard
    // const handleDelete =(id) =>{
    //         if(activeBtn === 'cart'){
    //             const UpdateCart = AddToCart.filter(items => items.product_id !== id)
    //             setAddToCart(UpdateCart)
                
    //         }else{
    //             const UpdateWish = WishList.filter(items => items.product_id !== id)
    //             setAddToCart(UpdateWish)
    //         }
    ...
    return (
        <div>
                ...
                <div className='flex justify-around items-center gap-6'>
                   {
                    activeBtn === 'cart' &&(
                        <>
                            ...
                            <div>
                                <button  onClick={()=>document.getElementById('my_modal_3').showModal()} className='btn border-2  px-16 rounded-full bg-[#9538E2] text-white'>Purchase</button>
                                {/* modal */}
                                ...
                                <div className="modal-box text-center ">
                                    ...
                                    <form method="dialog">
                                        <button className="btn px-14 rounded-full bg-purple-600 text-white ">Close</button>
                                    </form>
                                </div>
                                </dialog>
                            </div>
            ...
            <div>
                ...
                <div>
                    {AddToCart.length === 0 ? (
                        <h2 className='text-center text-3xl font-bold'>Cart is empty</h2>
                    ) : (
                    ...
                    <div>
                        {WishList.length === 0 ? (
                            <h2 className='text-center text-3xl font-bold py-10'>Wish list is empty</h2>
                        ) : (
